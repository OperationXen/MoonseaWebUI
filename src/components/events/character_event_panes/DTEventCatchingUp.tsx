...
export function DTEventCatchingUp(props: PropsType) {
  ...
  const [title, setTitle] = useState(event ? event.title : "Catching up");
  const [details, setDetails] = useState(event ? event.details : "");
  const [gold, setGold] = useState(event ? event.gold_change : 0);
  const [downtime, setDowntime] = useState(event ? event.downtime_change : -10);
...
export default DTEventCatchingUp;
